\documentclass[fontsize=12pt,b5paper,pagesize=auto,openany]{book}
% \usepackage[Conny]{fncychap}
\usepackage{imakeidx}

\usepackage[colorlinks = true,
            linkcolor = black,
            urlcolor  = blue,
            citecolor = black,
            anchorcolor = black,
            bookmarksnumbered = true]{hyperref}

\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{graphicx} % Required for inserting images
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{microtype}
\usepackage{bookmark}
\usepackage{setspace}
\usepackage[titles]{tocloft}
\usepackage{indentfirst}
\frenchspacing
\usepackage{csquotes}
\MakeOuterQuote{"}
\makeindex[intoc, options= -s indexstyle.ist]

%---- create the axioms enviroment -%

\newtheoremstyle{axiom} % name
    {\topsep}                    % Space above
    {\topsep}                    % Space below
    {\itshape\addtolength{\leftskip}{\parindent}}                   % Body font
    {}                           % Indent amount
    {\bfseries}                   % Theorem head font
    {}                          % Punctuation after theorem head
    {.5em}                       % Space after theorem head
    {#3}  % Theorem head spec (can be left empty, meaning ‘normal’)

\theoremstyle{axiom}
\newtheorem*{named}{}

\newtheoremstyle{exc} % name
    {\topsep}                    % Space above
    {\topsep}                    % Space below
    {\addtolength{\leftskip}{\parindent}}                   % Body font
    {}                           % Indent amount
    {\scshape}                   % Theorem head font
    {.}                          % Punctuation after theorem head
    {.5em}                       % Space after theorem head
    {}  % Theorem head spec (can be left empty, meaning ‘normal’)

\theoremstyle{exc}
\newtheorem*{exercise}{Exercise}

\newcommand{\subtitle}[1] {\textsc{#1}.\hspace{.5em}}
\newcommand{\subtitleup}[1] {#1.\hspace{.5em}}

%---- Package to create the C for collection of sets - %
\usepackage{fontspec}
\usepackage{unicode-math}
\setmainfont{STIX Two Text}
\setmathfont{STIX Two Math}
\setmathfont{STIX Two Math}[range={scr,bfscr},StylisticSet=01]
% ---------------------------------------------------- %


%---- Custom lists ----------------------------------- %
\usepackage{enumitem}
% ---------------------------------------------------- %
\usepackage{emptypage}
%---- Set up Header----------------------------------- %
\usepackage{fancyhdr}
\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{#1}}{}}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE, RO]{\thepage}
\fancyhead[CE]{\MakeUppercase{Naive Set Theory}}
\fancyhead[CO]{\MakeUppercase{\leftmark}}
\renewcommand{\headrulewidth}{0.1pt}
% ---------------------------------------------------- %

\usepackage{titlesec}
\titleformat
{\chapter} % command
[display] % shape
{\filcenter\large} % format
{
    \MakeUppercase{\chaptername\ \thechapter}\\
    \rule{0.3\textwidth}{1pt}
} % label
{1ex} % sep
{
    \LARGE\bfseries
    \MakeUppercase
} % before-code
[
    \vspace{-0.75ex}%
    \rule{0.8\textwidth}{2pt}
] % after-code

\titlespacing{\chapter}{0 pt}{0.5\baselineskip}{32 pt}

%------------------------------%
\begin{document}

\frontmatter

\begin{titlepage}
 \vspace*{1cm}
 {\huge\raggedright Naive Set Theory\par}
 \noindent\hrulefill\par
 {\LARGE\raggedleft Paul R. Halmos\textsuperscript{\textdagger}\par}
 \vfill
 {\Large\raggedright\textsuperscript{\textdagger}Professor of Mathematics, University of Michigan\par}
\end{titlepage}

\setlength{\headheight}{14.49998pt}
\setlength{\parindent}{0.75\parindent}

\setcounter{secnumdepth}{-1}
% PREFACES
\chapter{Preface by the Editor} 
\input{Chapters/Editor_Preface}

\chapter{Preface by the Author} 
\input{Chapters/Preface}

\clearpage\phantomsection

\newlength{\numlen}
\setlength{\numlen}{1em}
\renewcommand{\cftchapfont}{\scshape\bfseries}
\renewcommand{\cftchappresnum}{\hfill}
\renewcommand{\cftchapaftersnum}{\hspace*{\numlen}}
\addtolength{\cftchapnumwidth}{\numlen}
\setlength{\cftparskip}{-10pt}

\pdfbookmark{\contentsname}{toc}
\tableofcontents

\mainmatter
\fancyhead[RE, LO]{\textsc{Ch.\ \thechapter}}

\setcounter{secnumdepth}{0}

% CHAPTER 1
\chapter{The Axiom of Extension}
\input{Chapters/Chapter_1}

% CHAPTER 2
\chapter{The Axiom of Specification}
\input{Chapters/Chapter_2}

% CHAPTER 3
\chapter{Unordered Pairs}
\input{Chapters/Chapter_3}

% CHAPTER 4
\chapter{Unions and Intersections}
\input{Chapters/Chapter_4}

% CHAPTER 5
\chapter{Complements and Powers} 
\input{Chapters/Chapter_5}

% CHAPTER 6
\chapter{Ordered Pairs}
\input{Chapters/Chapter_6}

% CHAPTER 7
\chapter{Relations}
\input{Chapters/Chapter_7}

% CHAPTER 8
\chapter{Functions}
\input{Chapters/Chapter_8}

% CHAPTER 9
\chapter{Families}
\input{Chapters/Chapter_9}

% CHAPTER 10
\chapter{Inverses and Composites}
\input{Chapters/Chapter_10}

% CHAPTER 11
\chapter{Numbers}
\input{Chapters/Chapter_11}

% CHAPTER 12
\chapter{The Peano Axioms}
\input{Chapters/Chapter_12}

% CHAPTER 13
\chapter{Arithmetic}
\label{ch:arithmetic}
\input{Chapters/Chapter_13}

% CHAPTER 14
\chapter{Order}
\input{Chapters/Chapter_14}

% CHAPTER 15
\chapter{The Axiom Of Choice}
\label{ch:choice}
\input{Chapters/Chapter_15}

% CHAPTER 16
\chapter{Zorn's Lemma}
\input{Chapters/Chapter_16}

% CHAPTER 17
\chapter{Well Ordering}
\input{Chapters/Chapter_17}

% CHAPTER 18
\chapter{Transfinite Recursion} 
\input{Chapters/Chapter_18}

% CHAPTER 19
\chapter{Ordinal Numbers}
\input{Chapters/Chapter_19}

% CHAPTER 20
\chapter{Sets of Ordinal Numbers}
\input{Chapters/Chapter_20}

% CHAPTER 21
\chapter{Ordinal Arithmetic}
\input{Chapters/Chapter_21}

% CHAPTER 22
\chapter{The Schröder-Bernstein Theorem}
\input{Chapters/Chapter_22}

% CHAPTER 23
\chapter{Countable Sets}
\input{Chapters/Chapter_23}

% CHAPTER 24
\chapter{Cardinal Arithmetic}
\input{Chapters/Chapter_24}

% CHAPTER 25
\chapter{Cardinal Numbers}
\input{Chapters/Chapter_25}

\fancyhead[RE, LO]{}

% INDEX
\printindex

\end{document}
